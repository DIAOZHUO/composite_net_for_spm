...
class YOLO:
    ...
    def __init__(self, model: Union[str, Path, dict] = 'yolov8n.pt', task=None) -> None:
        """
    ...
    def _load(self, weights: Union[dict, str], task=None):
        """
        Initializes a new model and infers the task type from the model head.
        ...
        if type(weights) == dict:
            self.model, self.ckpt = self.attempt_load_one_weight_from_dict(weights)
    ...
    @staticmethod
    def attempt_load_one_weight_from_dict(weight: dict, device=None, inplace=True, fuse=False):
        """Loads a single model weights."""
        ckpt = weight  # load ckpt
        args = {**DEFAULT_CFG_DICT,
                **(ckpt.get('train_args', {}))}  # combine model and default args, preferring model args
        model = (ckpt.get('ema') or ckpt['model']).to(device).float()  # FP32 model

        # Model compatibility updates
        model.args = {k: v for k, v in args.items() if k in DEFAULT_CFG_KEYS}  # attach args to model
        model.task = guess_model_task(model)
        model.pt_path = None  # attach *.pt file path to model
